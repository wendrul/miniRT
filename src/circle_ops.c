...
/*   Updated: 2020/09/24 15:32:01 by ede-thom         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char*	check_circle_args(t_parse_args parsed)
{
	...
	if (args[7] > 255 || args[8] > 255 || args[9] > 255 ||
		...
		return ("Invalid RGB color values for circle (must be values between 0 and 255 per color).");
	return (NULL);
}
...
t_circle	create_circle(t_parse_args parsed)
{
	...
	circle.color = rgb_to_int(new_color(parsed.args[7], parsed.args[8], parsed.args[9]));
	...
	circle.eclipses = circle_eclipses_light;
...
t_point     circle_intersection(t_circle circle, t_vect ray, t_point start)
{
    ...
    if (dot(circle.normal, ray) == 0)
        return (result);
    r1 = (dot(circle.center, circle.normal) - dot(circle.normal, start)) / dot(circle.normal, ray);
    if (r1 <= 0)
        return (result);
...
int		    get_circle_side(t_point point, t_circle circle)
{
	float dist;
	...
	norm_point = add(circle.center, circle.normal);
...
int			circle_eclipses_light(t_point inter, t_circle circle, t_point light)
{
	t_vect	inter_to_spot;
	t_vect	inter_to_cir;
	t_vect	cir_inter;

	if (circle.is_reflective)
		return (0);
	inter_to_spot = substract(light, inter);
	cir_inter = circle.intersection(circle, normalize(inter_to_spot), inter);
	inter_to_cir = substract(cir_inter, inter);
	if (norm(inter_to_spot) < norm(inter_to_cir))
		return(0);
	return (1);
}
